...
<script>
  ...
  let estDetailGridOptions = {
    ...
    /*components: {
      datePicker2: getDatePicker("dueDateOfEstimate")
    }*/
...
//  ======================일괄저장======================================================================
  ...
  batchSaveButton.addEventListener("click", () => {  //일괄저장을 클릭했을때
    ...
    if (newEstimateRowValue == "") { // 이게 어떻게하면 출력되는거지
      Swal.fire({
   ...
   //console.log(estGridOptions.getSelectedRowData())
    ...
    newEstimateRowValue = JSON.stringify(newEstimateRowValue);  // 받아온 값들을 JSON형태로 바꾸어준다고 생각=> 문자열로 변환
      console.log("newEstimateRowValue"+newEstimateRowValue); // 왜 다 입력이 되어야 콘솔창에 출력이 될까
    Swal.fire({
      title: "견적을 등록하시겠습니까?",
      icon: 'warning',
